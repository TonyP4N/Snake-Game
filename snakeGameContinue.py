...
def moveSnake():
    ...
    if collision(sHeadPos, foodPos):
        moveFood()
        growSnake()
    for i in range(1, len(snake)):
        if collision(sHeadPos, canvas.coords(snake[i])):
            gameOver = True
            canvas.create_text(width / 2, height / 2, fill="dark blue", font=("Times 20 italic bold", 30), text="Game Over!")
            timer.destroy()
            if score >= 390:
    ...
    if 'gameOver' not in locals():
        top.after(speed, moveSnake)
    if speed == 999999:
        canvas.create_text(width / 2, height / 2, fill="dark blue", font=("Times 20 italic bold", 30), text="Time out!")


def respawn():
    top.destroy()
    os.system("Snake_Game.py")
...
def timeConsumer():
    ...
    variable.set('%02d:%02d' % (minutes, seconds + seconds1))
...
#loader
loader1 = open("saver.txt", "r")
loader = loader1.readlines()
speed1 = int(loader[0].strip())
seconds1 = int(loader[1].strip())
score1 = int(loader[2].strip())
...
score = score1
txt = "Score:" + str(score1)
scoreText = canvas.create_text(width / 2, 10, fill="black", font=("Times 25 italic bold"), text=txt)
